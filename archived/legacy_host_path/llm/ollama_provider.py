...
class OllamaProvider(LLMProvider):
    ...
    def _request(self, method: str, path: str, **kwargs: Any) -> requests.Response:
        ...
        try:
            response = self._session.request(method, url, timeout=self._settings.llm_timeout, **kwargs)
        except requests.Timeout as exc:
            raise ProviderTimeoutError(
                f"Ollama request to {url} timed out after {self._settings.llm_timeout} seconds."
            ) from exc
